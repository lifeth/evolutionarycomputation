...
//      Tracer.addTracer(goal, tracer);  // Uncomment if you want to trace the function evaluations
    ...
    public static Hashtable<BitArray,Integer> histogram( Solution<BitArray>[] pop ){
        Hashtable<BitArray,Integer> hash = new Hashtable<BitArray,Integer>();
        for( int i=0; i<pop.length; i++ ){
            Integer k = hash.get(pop[i].object());
            hash.put(pop[i].object(), k!=null?(k+1):1);
        }
        
        System.out.println("################################");
        for( Entry<BitArray, Integer> entry : hash.entrySet()) {
    ...
    public static void scheme_theorem(){
        // Search Space definition
        int DIM = 100;
        ...
        BitArray[] pop = new BitArray[POP_SIZE];
        ...
        for( int i=1; i<=MAX_ITER; i++ ){
            solution = selection.pick(POP_SIZE, solution);
            if( i%10 == 0 ) histogram(solution);
        }
    }
    ...
    public static void binary(){
        ...
        PopulationSearch<BitArray,Double> search = 
                factory.generational_ga(POPSIZE, new Tournament<BitArray>(4), mutation, xover, 0.0, MAXITERS );
