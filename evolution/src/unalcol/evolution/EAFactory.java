...
import funico.GPIterativePopulationSearch;
import ga.epimarks.GAIterativePopulationSearch;
...
public class EAFactory<T> {
	...
	public PopulationSearch<T,Double>	generational_ga(
		...
		return new GAIterativePopulationSearch<T,Double>(
				new GAStep<T>( mu, parent_selection, mutation, xover, xover_probability, true),
				tC );	
	}
	...
	public PopulationSearch<T, Double> geneticProgramming(int mu, Selection<T> parent_selection,
			Variation_1_1<T> mutation, Variation_2_2<T> xover, double xover_probability, Predicate<Population<T>> tC) {

		return new GPIterativePopulationSearch<T, Double>(
				new GAStep<T>(mu, parent_selection, mutation, xover, xover_probability, false), tC);
	}

	public PopulationSearch<T, Double> geneticProgramming(int mu, Selection<T> parent_selection,
			Variation_1_1<T> mutation, Variation_2_2<T> xover, double xover_probability, int MAXITERS) {
		return geneticProgramming(mu, parent_selection, mutation, xover, xover_probability,
				new ForLoopCondition<Population<T>>(MAXITERS));
	}
